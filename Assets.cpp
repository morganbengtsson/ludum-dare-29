...
Assets::Assets() {
    ...
    models_.insert(MeshPair("Empty.obj", shared_ptr<Mesh>(new Mesh(vertices.begin(),
            vertices.end(),
            elements.begin(),
            elements.end()))));
...
mo::Mesh * Assets::loadMesh(std::string path) {
    using namespace std;
    using namespace mo;
    ...
    if (models_.find(path) == models_.end()) {
        ...
        models_.insert(MeshPair(path, std::shared_ptr<mo::Mesh>(new mo::Mesh(vertices.begin(),
                vertices.end(),
                elements.begin(),
                elements.end()))));
        return models_.at(path).get();
    } else {
        return models_.at(path).get();
    }
}

mo::Texture2D * Assets::loadTexture(std::string path) {
    using namespace mo;
    if (textures_.find(path) == textures_.end()) {
        ...
        textures_.insert(TexturePair(path, std::shared_ptr<Texture2D>(new Texture2D(texels.begin(), texels.end(), width, height))));
        return textures_.at(path).get();
    } else {
        return textures_.at(path).get();
    }
}
